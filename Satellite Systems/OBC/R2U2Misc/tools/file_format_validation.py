...
with open("input.csv") as csvfile:
    ...
    # tests the first column of data 
    for row in range(len(data)-1):
        r = len(data)-1
        num = data[row+1][0]
        test = int(num)
        
        # tests if the first column is within range
        if (test >= 0) and (test <= 4294967295):
            pass
        else:
            print('Invalid file format')
            break
        
         # tests if the data is monotomically increasing 
        if (row + 2) <= r:
            num1 = data[row+2][0]
        elif (row + 2) == (r+1):
            numstr = data[row+1][0]  

...
HAL_StatusTypeDef TC_2(uint32_t sec, uint16_t millisec){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 7);
	return status;
}
...
HAL_StatusTypeDef TC_7(uint8_t timeout){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 2);
	return status;
}
...
HAL_StatusTypeDef TC_11(uint8_t cubeControl_signal, uint8_t cubeControl_motor, uint8_t cubeSense_power, uint8_t cubeStar_power, uint8_t cubeWheel1_power, uint8_t cubeWheel2_power, uint8_t cubeWheel3_power, uint8_t motor_power, uint8_t GPS_power){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 4);
	return status;
}
...
HAL_StatusTypeDef TC_13(uint8_t mode, uint8_t flag, uint16_t timeout){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 5);
	return status;
}
...
HAL_StatusTypeDef TC_17(int16_t x_speed, int16_t y_speed, int16_t z_speed){
	uint8_t data[7];
	data[0] = 17;
    memcpy(&data[1], &x_speed, 2);
    memcpy(&data[3], &y_speed, 2);
    memcpy(&data[5], &z_speed, 2);
...
HAL_StatusTypeDef TC_34(float channel_1, float channel_2, float channel_3, float matrix_11, float matrix_22, float matrix_33){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 13);
	return status;
}
...
HAL_StatusTypeDef TC_44(float nadir_Sensor, float magnetometer, float star_tracker, uint8_t use_sun_sensor, uint8_t use_nadir_sensor,uint8_t use_CSS,uint8_t use_star_tracker,uint8_t magnetometer_mode,uint8_t sampling_period){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 15);
	return status;
}
...
HAL_StatusTypeDef TC_45(double inclination, double eccentricity, double right_ascension, double argument, double b_star, double mean_motion, double mean_anomaly, double epoch){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 65);
	return status;
}
...
HAL_StatusTypeDef TC_63(){
	...
	HAL_StatusTypeDef status = ADCS_WRITE(data, 1);
	return status;
}
...
HAL_StatusTypeDef TLM_140(uint32_t* sec, uint16_t* millisec){
	...
	HAL_StatusTypeDef status = ADCS_READ(&TLM_ID, data, 1, 6);
 ...
 * @param x X wheel speed in rpm
 * @param y Y wheel speed in rpm
 * @param z Z wheel speed in rpm
 */
HAL_StatusTypeDef TLM_156(int16_t* x, int16_t* y, int16_t* z){
	uint8_t data[6];
	uint8_t TLM_ID = 156;
...
HAL_StatusTypeDef TLM_169(uint8_t* CSS_7, uint8_t* CSS_8, uint8_t* CSS_9, uint8_t* CSS_10){
	...
	HAL_StatusTypeDef status = ADCS_READ(&TLM_ID, data, 1, 4);
...
/********************************************** HELPER FUNCTIONS **********************************************************************************/
/**
 * @brief Sends a read command to the ADCS module over a given I2C instance and reads back a response
...
*/
HAL_StatusTypeDef ADCS_READ(uint8_t command[], uint8_t* data_ptr, uint8_t in_byte, uint8_t out_byte){
        HAL_StatusTypeDef status = HAL_ERROR;
        status = HAL_I2C_Master_Transmit(&hi2c1, (uint16_t) (ADCS_I2C_ADDRESS) << 1, command, in_byte, ADCS_I2C_TIMEOUT);
        if(status != HAL_OK){
            return status;
        }
        status = HAL_I2C_Master_Receive(&hi2c1, (uint16_t) (ADCS_I2C_ADDRESS) << 1, data_ptr, out_byte, ADCS_I2C_TIMEOUT);
        return status;
}

/**
 * @brief Sends a write command to the ADCS module over a given I2C instance
 * @param command  :the command to be used on the transceiver
 * @param in_byte  :The size of the command that is being sent
 * @retval Returns if the hal function call succeeded, or if it failed
*/
HAL_StatusTypeDef ADCS_WRITE(uint8_t command[], uint8_t in_byte){
        HAL_StatusTypeDef status = HAL_ERROR;
        status = HAL_I2C_Master_Transmit(&hi2c1, (uint16_t) (ADCS_I2C_ADDRESS) << 1, command, in_byte, ADCS_I2C_TIMEOUT);
        return status;
}
